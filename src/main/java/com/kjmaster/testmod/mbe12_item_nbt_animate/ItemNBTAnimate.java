...
public class ItemNBTAnimate extends Item {
    ...
    public ItemNBTAnimate() {
        ...
        if (!TestMod.proxy.isDedicatedServer()) {
    ...
    public boolean hasEffect(ItemStack stack) {
        ...
        return nbtTagCompound != null && nbtTagCompound.hasKey("Bound");
    }
    ...
    // how long the player needs to hold down the right button in order to activate the gem, in ticks (1 tick = 1/20 sec)
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (bound) {
            playerIn.setActiveHand(handIn); // start the charge up sequence
            return new ActionResult<>(EnumActionResult.SUCCESS, itemStackIn);
        } else {
            if (worldIn.isRemote) { // only on the client side, else you will get two messages...
                final boolean PRINT_IN_CHAT_WINDOW = true;
                playerIn.sendStatusMessage(new TextComponentString("Gem doesn't have a stored location!"
                        + " Shift right click to store your current location"), PRINT_IN_CHAT_WINDOW);
            }
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase entityLiving) {
        NBTTagCompound nbtTagCompound = stack.getTagCompound();
        if (nbtTagCompound == null || !nbtTagCompound.hasKey("Bound") || !nbtTagCompound.getBoolean("Bound")) {
            return stack;
        }
        ...
        if (worldIn.isRemote) { // client side
            worldIn.playSound(x, y, z, SoundEvents.ENTITY_ENDERMEN_TELEPORT, SoundCategory.PLAYERS,
                    1.0F, 1.0F, false);
        } else { // server side
            if (entityLiving instanceof EntityPlayerMP) { // should be an EntityPlayerMP; check to avoid possible crash
                ...
                worldIn.playSound(dontPlayForThisPlayer, x, y, z, SoundEvents.ENTITY_ENDERMEN_TELEPORT, SoundCategory.PLAYERS,
                        1.0F, 1.0F);
            }
        }
