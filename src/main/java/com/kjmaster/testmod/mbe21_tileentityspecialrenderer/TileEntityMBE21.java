...
public class TileEntityMBE21 extends TileEntity {
    ...
    public static final Color INVALID_COLOUR = null;
    private Color gemColour = INVALID_COLOUR;
    ...
    public void setGemColour(Color gemColour) {
    ...
    // In this case, we only need to store the gem colour. For other types of data check out MBE20.
    @Override
    public NBTTagCompound writeToNBT(NBTTagCompound compound) {
        super.writeToNBT(compound); // required to save tile location
        if (gemColour != INVALID_COLOUR) {
            compound.setInteger("gemColour", gemColour.getRGB());
        }
        return compound;
    }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        super.readFromNBT(compound); // Required to load tiles location

        // important rule: never trust the data you read from NBT, make sure it can't cause a crash

        final int NBT_INT_ID = 3; // see NBTBase.createNewByType
        Color readGemColour = INVALID_COLOUR;
        if (compound.hasKey("gemColour", NBT_INT_ID)) { // check if the key exists and is an Int. You can omit this if default of 0 is ok.
            int colourRGB = compound.getInteger("gemColour");
            readGemColour = new Color(colourRGB);
        }
